...
import Module from 'components/sanity-ui/modules/module';
...
const FULL_WIDTH_MODULE_TYPES: SanityModule['_type'][] = ['module.callout', 'module.callToAction'];
          ...
          return (
            <li
              className={clsx([
                ...
                FULL_WIDTH_MODULE_TYPES.includes(item._type) ? 'md:col-span-2' : 'md:col-span-1'
              ])}
              key={item._key}
            >
              <div className={clsx(isProductModule ? productWidth : 'w-full')}>
                <Module imageAspectClassName={productImageAspect} module={item} />
              </div>
            </li>
          );
        } else {
...
const isModule = (item: SanityModule): item is SanityModule => {
